...
def build_diag(Hb_max, Kni_max, breaks):
    ...
    for i in breaks:
        if i not in keep:
            print('Diagonal requirment will disconnect Product Graph', flush=True)
    return keep

def remove_unnecessary_nodes_in_P(P, keep, scc):
...
def get_results_from_string(string, network_filename):
    ...
    try:
        with open(txt_filename,"r") as f:
            network = f.read()
    except: # Make txt file for network, needed to build DSGRN database
